(function () {
  ...
  const HELP_TEXT_REPLACEMENTS = new Map([
    ...
    ['To determine a value, take the number of threads and multiply it by 100.', '적절한 값을 정하려면 스레드 수에 100을 곱하세요.'],
    ['For example, on a quad core system without hyperthreading', '예를 들어 하이퍼스레딩이 없는 4코어 시스템에서는'],
    ['there is', '사용 가능한 값은'],
    ['available.', '입니다.'],
    ['If you do not want to limit CPU usage, set the value to 0.', 'CPU 사용량을 제한하지 않으려면 값을 0으로 설정하세요.'],
    ['To limit a server to using half of a single thread, you would set the value to 50.', '한 스레드의 절반만 사용하도록 제한하려면 값을 50으로 설정하세요.'],
    ['To allow a server to use up to two threads, set the value to 200.', '최대 두 스레드를 사용하도록 하려면 값을 200으로 설정하세요.'],
    ...
    ['Example: 0, 0-1,3, or 0,1,3,4.', '예: 0, 0-1,3, 0,1,3,4'],
