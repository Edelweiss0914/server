...
### 4.0 레거시 systemd → Docker Compose 1회 전환

Gateway가 아직 `cheeze-portal-api`, `cheeze-control-api`, `cheeze-ai-queue`, `cheeze-nextjs`를
systemd로 실행 중이라면 **먼저 중지/비활성화**해야 한다. 그렇지 않으면 Docker 포트 바인딩 충돌과
혼합 네트워크 문제로 `502 Bad Gateway`가 반복된다.
...
- 레거시 systemd 서비스 중지 및 비활성화
- `portal-api`, `control-api`, `ai-queue`, `web`, `nginx` 이미지 빌드
- Docker Compose 서비스 기동
- `11435`, `11436`, `11437`, `3000` 헬스 확인

전환 후에는 Python API와 Next.js를 **systemd로 다시 올리지 말고** Compose만 사용한다.
