...
| `GET` | `/admin/idle/status` | 유휴 상태 프록시 | `admin` role |
| `GET` | `/admin/hibernate/debug` | 하이버네이션 조건 디버그 프록시 | `admin` role |
| `GET`/`POST`/`DELETE` | `/admin/no-sleep` | no-sleep 플래그 조회/설정/해제 | `admin` role |
| `GET` | `/admin/system/resources` | Backend PC 리소스 (CPU/메모리/디스크) | `admin` role |
| `GET` | `/admin/gateway/resources` | Gateway VM 리소스 (CPU/메모리/디스크) | `admin` role |
