...
> **참고**: 이벤트 기반 유예 기간은 **계획됨 (미구현)**입니다. 현재는 `inhibit_schedule` 시간 범위 조건만 구현되어 있습니다.

| 이벤트 | 유예 시간 |
|--------|----------|
| 에이전트 시작 | 180초 — 계획됨 (미구현) |
| 시스템 재개(resume) | 180초 — 계획됨 (미구현) |
| 서비스 시작 요청 | 600초 — 계획됨 (미구현) |
...
> **참고**: RCON 경고 브로드캐스트 및 유휴 5분 전 자동 저장 트리거는 **계획됨 (미구현)**입니다. 현재는 유휴 타임아웃 만료 시 자동 중지만 동작합니다.

### 경고 및 동작 타임라인

| 중지까지 남은 시간 | 동작 |
|-------------------|------|
| 30분 | RCON 경고 브로드캐스트 — **계획됨 (미구현)** |
| 20분 | RCON 경고 브로드캐스트 — **계획됨 (미구현)** |
| 10분 | RCON 경고 브로드캐스트 — **계획됨 (미구현)** |
| 5분 | RCON 경고 브로드캐스트 + 자동 저장 트리거 — **계획됨 (미구현)** |
| 1분 | RCON 경고 브로드캐스트 — **계획됨 (미구현)** |
| 0분 | 서비스 자동 중지 |
...
> **계획됨 (미구현)**: 아래 자동 저장 기능은 현재 구현되지 않았습니다.
